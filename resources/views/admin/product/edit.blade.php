    ...
    <div class="settings-form">
        ...
        <form method="POST" action="{{ route('admin.product.update') }}" enctype="multipart/form-data" id="editForm">
                    ...
                    <div id="galleryPreviewStandalone" class="mt-2" style="position: relative; width: 110px;">
                        ...
                        @if ($firstVariant && $firstVariant->gallery_images)
                            <div class="gallery-preview-item">
                                <img src="{{ asset('public/gallery_images/' . $firstVariant->gallery_images) }}"
                                    width="100" class="img-thumbnail">

                                {{-- Changed onclick to a simpler global function --}}
                                <button type="button" class="btn-xs-delete"
                                    onclick="removeGlobalGalleryImage(this)">×</button>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="row">
                ...
                <div class="col-md-3 mb-3">
                    <label>GST</label>
                    <input type="number" name="gst" placeholder="Ex: 18%" class="form-control" value="{{ $product->gst }}">
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                Best
                Sellers
                <input type="checkbox" name="new_arrival" value="1" {{ $product->new_arrival ? 'checked' : '' }}>
                New
                Arrival
            ...
            <div class="row">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="number" name="unit_value" class="form-control" min="1"
                        value="{{ old('unit_value', $product->unit_value ?? 1) }}" required>
                </div>
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <select name="unit_type" class="form-control" required>
                        ...
                        <option value="Piece"
                            {{ old('unit_type', $product->unit_type ?? '') == 'Piece' ? 'selected' : '' }}>
                            Piece
                        </option>

                        <option value="Pieces"
                            {{ old('unit_type', $product->unit_type ?? '') == 'Pieces' ? 'selected' : '' }}>
                            Pieces
                        </option>

                        <option value="Pair"
                            {{ old('unit_type', $product->unit_type ?? '') == 'Pair' ? 'selected' : '' }}>
                            Pair
                        </option>

                        <option value="Set"
                            {{ old('unit_type', $product->unit_type ?? '') == 'Set' ? 'selected' : '' }}>
                            Set
                        </option>

                        <option value="Pack"
                            {{ old('unit_type', $product->unit_type ?? '') == 'Pack' ? 'selected' : '' }}>
                            Pack
                        </option>

                        <option value="Box"
                            {{ old('unit_type', $product->unit_type ?? '') == 'Box' ? 'selected' : '' }}>
                            Box
                        </option>

                        <option value="Bottle"
                            {{ old('unit_type', $product->unit_type ?? '') == 'Bottle' ? 'selected' : '' }}>
                            Bottle
                        </option>

                        <option value="Kg"
                            {{ old('unit_type', $product->unit_type ?? '') == 'Kg' ? 'selected' : '' }}>
                            Kg
                        </option>

                        <option value="Gram"
                            {{ old('unit_type', $product->unit_type ?? '') == 'Gram' ? 'selected' : '' }}>
                            Gram
                        </option>
                        ...
                        <option value="Meter"
                            {{ old('unit_type', $product->unit_type ?? '') == 'Meter' ? 'selected' : '' }}>
                            Meter
                        </option>

                        <option value="Roll"
                            {{ old('unit_type', $product->unit_type ?? '') == 'Roll' ? 'selected' : '' }}>
                            Roll
                        </option>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-3">
                    <button type="button" id="addVariantBtn" class="btn btn-success w-100">
                        <i class="mdi mdi-plus"></i> Add Variant</button>
