...
@section('admin')
    ...
    <div class="card-header">
        ...
        <button type="button" class="btn" style=" background-color: #001E40 !important;color:#fff"
            onclick="window.location.href = '{{ route('admin.product.color.add') }}'">
            Add New
        </button>
    ...
    <div class="card-body">
        <div class="table-responsive" style="overflow-x:auto;">
            <table id="example4" class="display table table-bordered verticle-middle table-striped table-responsive-sm"
                ...
                <tbody>
                    @foreach ($color_code as $key => $item)
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>{{ $item->color }} ( {{ $item->code }} )</td>
                            <td>
                                <div class="color-box me-2" style="background-color: {{ $item->code }};padding:10px;">
                                </div> <!-- Display color box -->
                            </td>
                            <td>
                                <div class="d-flex">
                                    ...
                                    <a href="{{ route('product.color.delete', $item->id) }}"
