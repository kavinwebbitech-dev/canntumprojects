  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="subCategoryModalLabel">Add Blog SubCategory</h5>
        ...
        <table id="example4" class="display table table-bordered verticle-middle table-striped table-responsive-sm" style="min-width: 845px">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $key+1 }}</td>
                    <td>
                        @php $category = App\Models\BlogCategory::where('id',$item->category_id)->first(); @endphp
                        {{ $category->name ?? '' }}
                    </td>
                    <td>{{ $item->name }}</td>
                    ...
                    <td>
						<div class="d-flex">
                            ...
                            <button class="btn btn-primary shadow btn-xs sharp editSubCategory" data-id="{{ $item->id }}"><i class="fas fa-pencil-alt"></i></button>
						</div>
					</td>
                </tr>
                @endforeach
...
<script>
  $(document).ready(function() {
    function saveSubCategory() {
      ...
      $.ajax({
        url: '{{ route("admin.blog.subcategory.add") }}',
        type: 'POST',
        data: formData,
...
$('.editSubCategory').click(function() {
    ...
    $.ajax({
        url: '{{ url("/blog-subcategory-edit/") }}/' + categoryId,
        type: 'GET',
        success: function(response) {
            $('#editSubsubCategoryModal').html(response);
...
$(document).on('click', '#saveEditedSubCategory', function() {
        console.log('Save button clicked'); 
        var formData = $('#editSubCategoryForm').serialize();
        console.log('Form data:', formData); 
        $.ajax({
            url: '{{ route("admin.blog.subcategory.update") }}',
            type: 'POST',
            data: formData,
            success: function(response) {
                console.log(response);
