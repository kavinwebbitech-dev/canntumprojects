...
class ProductController extends Controller
{
    ...
    //         'quantity'       => 'required|integer|min:1', // Added required here
    //         'file1'          => 'nullable|image|mimes:jpeg,png,jpg,gif,webp|max:5120',
    //         'file2.*'        => 'nullable|image|mimes:jpeg,png,jpg,gif,webp|max:5120',
    //         'variants'       => 'nullable|array',
    //         'variants.*.color_id' => 'required_with:variants',
    //         'variants.*.size_id'  => 'required_with:variants',
    ...
    //         $product->orginal_rate = $request->actual_price; // Note: verify if it's 'orginal' or 'original' in DB
    ...
    //             $product_image = $request->file('file1');
    //             $fileName = date('YmdHis') . '_' . uniqid() . '.' . $product_image->extension();
    //             $product_image->move(public_path('product_images'), $fileName);
    //             $product->product_img = $fileName;
    //         }
    //         $product->save();

    //         // Handle Gallery Images (file2)
    ...
    //                     foreach ($request->file("variants.{$index}.images") as $vImg) {
    //                         $vImgName = date('YmdHis') . '_' . uniqid() . '.' . $vImg->extension();
    //                         $vImg->move(public_path('variant_images'), $vImgName);
    //                         $variant_images[] = $vImgName;
    //                     }
    //                 }
    ...
    //                 $product_details->color_id = $variant['color_id'];
    //                 $product_details->size_id = $variant['size_id'];
    //                 $product_details->price = $product->orginal_rate;
    //                 $product_details->quantity = $product->quantity;
    //                 $product_details->images = json_encode($variant_images);
    //                 $product_details->save();
    //             }
    //         } else {
    //             // No variants, save one record in details
    ...
    public function productStore(Request $request)
    {
        $request->validate([
            ...
            // ✅ FIXED: variant-based quantity validation
